...
def load_dataset_train(dataset_folder, candle_type_and_directory_save: dict):    
    list_dataset = []
...
def get_close_price_percent_of_last_days_result(days_result, features, labels):
...
def get_prices_percent_of_last_days_result(days_result, features, labels):
...
def get_high_low_prices_percent_of_last_days_result(days_result, features, labels):
...
def get_open_close_prices_percent_of_last_days_result_for_one_day_result(days_result, features, labels):
...
def get_open_close_prices_percent_of_last_days_result_for_multiple_days_result(days_result, features, labels):
    labels = swap_max_and_min_two_columns(labels, 1, 2)
...
def get_open_close_prices_percent_of_last_days_result(days_result, features, labels):
    if days_result > 1:
        return get_open_close_prices_percent_of_last_days_result_for_multiple_days_result(days_result, features, labels)
    return get_open_close_prices_percent_of_last_days_result_for_one_day_result(days_result, features, labels)


def get_max_in_open_close_prices_percent_of_last_days_result_for_multiple_days_result(days_result, features, labels):
    labels = swap_max_and_min_two_columns(labels, 1, 2)
...
def up_or_down_in_close_for_multiple_days_result(days_result, features, labels):
    difference = labels[:, 1:] - labels[:, :-1]
...
def up_or_down_in_close_for_one_days_result(days_result, features, labels):
    difference = labels[:, 1:] - labels[:, :-1]
